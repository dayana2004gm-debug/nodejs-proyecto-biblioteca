...
router.post("/", (req, res) => {
    ...
    const sql = `
        ...
        VALUES (?, ?, ?, ?, ?)
    `;
    pool.query(sql, [nombreeditorial, direccion, ciudad, pais, telefono], (error, result) => {
        if (error) {
            console.error(error);
...
router.put("/", (req, res) => {
    ...
    const sql = `
        ...
        WHERE ideditorial = ?
    `;
    pool.query(sql, [nombreeditorial, direccion, ciudad, pais, telefono, ideditorial], (error, result) => {
        if (error) {
            console.error(error);
...
router.delete("/:ideditorial", (req, res) => {
    ...
    pool.query(sql, [ideditorial], (error, result) => {
        if (error) {
            console.error(error);
